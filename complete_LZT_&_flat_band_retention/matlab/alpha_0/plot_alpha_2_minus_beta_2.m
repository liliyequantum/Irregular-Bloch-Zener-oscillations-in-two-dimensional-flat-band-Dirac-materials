function plot_alpha_2_minus_beta_2
...
if  k==int64(3/electric_field*num_step) || k==int64(5.5/electric_field*num_step) || k==int64(8/electric_field*num_step)
% if  k==int64(5.5/electric_field*num_step)
    
load(['./picture_data/p_alpha_2_minus_beta_2_t_max_',...
            num2str(t(k)),'_dt_',num2str(dt),'.mat'])
c=c+1;
figure(c)
surface(p_x,p_y,p_alpha_2_minus_beta_2,'EdgeColor','none')
hold on
...
colormap(parula(20));
cbh = colorbar;
cbh.Ticks = [-0.5,0,0.5] ; %Create 8 ticks from zero to 1
cbh.TickLabels = num2cell([-0.5,0,0.5]) ;  
cbh.FontSize = 25;
...
axis tight
saveas(gcf,['./picture/p_alpha_2_minus_beta_2_t_max_',num2str(t(k)),'_dt_',num2str(dt),'.png'])
close
