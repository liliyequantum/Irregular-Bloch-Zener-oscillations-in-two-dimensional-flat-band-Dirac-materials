...
p_x_size=300;
p_y_size=120;
p_x_1=linspace(0,1/sqrt(3)*2*pi,p_x_size);
...
% start = tic;
c=1;
checkNormalizationMax = zeros(length(t)-1,2);
checkNormalizationMin = zeros(length(t)-1,2);
...
%     tic
    ...
    J_inter(k+1,1)=(sum(J_p_inter_1)+sum(J_p_inter_2)).*dp_x.*dp_y;
    J_intra(k+1,1)=(sum(J_p_intra_1)+sum(J_p_intra_2)).*dp_x.*dp_y;
    ...
    't','p_x_1','p_x_2','p_y_1','p_y_2','J_p_inter_1','J_p_inter_2','J_p_intra_1','J_p_intra_2','p_x_size','p_y_size','electric_field','J_inter','J_intra','alpha_parameter','x_1','x_2')
        stop
    end  
%     disp(strcat("spend time: ", num2str(toc)," s")) 
    ...
    if k==selectTime(c) 
        ...
        save(['./picture/E_0_12_small_k_area/picture_data/width_',width_char,...
            ...
            'p_x_1','p_y_1','p_alpha_2_minus_beta_2_1','p_x_2','p_y_2','p_alpha_2_minus_beta_2_2')
